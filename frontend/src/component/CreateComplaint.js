...
export default function ComplaintTracking() {
  ...
  const handleCreateComplaint = () => {
    ...
    let priority = newComplaint.priority;

    if (newComplaint.status === "Pending" && priority !== "Medium") {
      setErrorMessage("Priority must be Medium when status is Pending.");
      return;
    }

    if (newComplaint.status === "Open" && priority !== "Low") {
      setErrorMessage("Priority must be Low when status is Open.");
      return;
    }

    if (newComplaint.status === "Solve" && priority !== "High") {
  ...
  useEffect(() => {
    ...
    if (newComplaint.priority === "High") {
      setNewComplaint(prev => ({ ...prev, status: "Solve" }));
    } else if (newComplaint.priority === "Medium") {
      setNewComplaint(prev => ({ ...prev, status: "Pending" }));
    } else if (newComplaint.priority === "Low") {
      setNewComplaint(prev => ({ ...prev, status: "Open" }));
    }
  }, [newComplaint.priority]); // Trigger this effect whenever priority changes
          ...
          <div className="table-responsive" style={{ border: "1px solid #ddd", borderRadius: "8px", boxShadow: "0px 0px 15px rgba(0, 0, 0, 0.1)", overflow: "hidden", backgroundColor: "#fff", padding: "20px", marginTop: "20px" }}>
            <Table striped hover responsive className="mt-3" style={{ width: "1100px" }}>
              <thead className="bg-light">
                <tr className="rmHead">
                  <th className="text-start" style={{ padding: "10px" }}>Complainer Name</th>
                  <th className="text-center" style={{ padding: "10px" }}>Complaint Name</th>
                  <th className="text-center" style={{ padding: "10px" }}>Description</th>
                  <th className="text-center" style={{ padding: "10px" }}>Unit Number</th>
                  <th className="text-center" style={{ padding: "10px" }}>Priority</th>
                  <th className="text-center" style={{ padding: "10px" }}>Status</th>
                  <th className="text-center" style={{ padding: "10px" }}>Actions</th>
                </tr>
              </thead>
              <tbody>
